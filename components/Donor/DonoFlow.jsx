...
const DonorFlow = () => {
  ...
  const [expiryHours, setExpiryHours] = useState(12); // New Expiry mechanic default 12 hours
  ...
  const submitDonation = async () => {
    // Attempt to grab email from localStorage to assign points
    let userEmail = "";
    try {
    ...
    formData.append("expiryHours", expiryHours); // Pushing expiry to backend
    ...
    try {
      ...
      if (res.ok) {
        toast.success("Donation submitted successfully! You earned 50 points.");
        setTimeout(() => {
          router.push('/donor/dashboard');
        }, 1000);
      } else {
        const errorData = await res.json();
        toast.error(errorData.message || "Failed to submit donation.");
      }
    } catch (error) {
      console.error("Error submitting donation:", error);
      toast.error("Network error submitting donation.");
    }
  };
  ...
  const handleDonorFormSubmit = (e) => {
    e.preventDefault();
    if (!foodFor) {
      toast.error("Please select who the food is for.");
      return;
    }
  ...
  const handleDonationLocationSubmit = async (e) => {
    ...
    if (success) {
      toast.success("Donation details submitted successfully! You earned 50 points.");
    }
  };
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {["individual", "restaurant", "event_organization"].map((type) => (
                <label
                  key={type}
                  className={`flex items-center justify-center p-2 rounded-lg text-sm cursor-pointer transition-all ${donorType === type
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100 p-6">
      <div className="bg-white shadow-lg rounded-lg p-8 w-full max-w-md">
        ...
        <form onSubmit={handleDonorFormSubmit}>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                {["veg", "nonveg"].map((type) => (
                  <label
                    key={type}
                    className={`flex items-center justify-center p-3 rounded-lg text-sm cursor-pointer transition-all border-2 ${foodType === type
                  ...
                  >
                    <input
                      ...
                      className="hidden"
                    />
                    <span className="capitalize font-bold">{type === "nonveg" ? "Non-Veg" : "Veg"}</span>
                  </label>
                ))}
              </div>
          ...
          <label className="block text-sm font-medium text-gray-700 mb-2">Food Expiry timeframe:</label>
          <div className="relative flex items-center border border-gray-300 rounded-lg bg-gray-50 mb-6">
            <span className="ml-3 text-orange-600 font-bold">⏳</span>
            <select
              className="w-full p-3 bg-transparent outline-none text-gray-700"
              value={expiryHours}
            ...
            >
              ...
              <option value="48">Expires in 2 Days</option>
            </select>
          </div>
